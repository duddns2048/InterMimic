...
class InterMimic(Humanoid_SMPLX):
    ...
    def _load_target_asset(self): # smplx
        ...
        for i, object_name in enumerate(self.object_name):
            ...
            if object_name in ['kettle', 'cup']:
                asset_options.vhacd_params.max_convex_hulls = 80
            elif object_name in ['chair']:
        ...
        if 'parahome' in self.motion_file[0]:
            self._static_target_asset = []
            self.static_object_points = []
            if 'cup' in self.motion_file[0]: 
                self.static_object_name = ['diningtable', 'desk']
            elif 'book' in self.motion_file[0]: 
                self.static_object_name = ['desk', 'bookshelf']
            elif 'chair' in self.motion_file[0]: 
                self.static_object_name = ['desk', 'diningtable']
            ...
            for static_object_name in self.static_object_name:
                ...
                self._static_target_asset.append(self.gym.load_asset(self.sim, asset_root, asset_file, asset_options))
                ...
                while object_points.shape[0] < 1024:
                    object_points = torch.cat([object_points, object_points[:1024 - object_points.shape[0]]], dim=0)
                self.static_object_points.append(to_torch(object_points))
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        if 'parahome' in self.motion_file[0]:
            ...
            static_obj_pose = gymapi.Transform()
            subj_num=self.cfg['env']['dataSub'][0][3:]
            object_transformations = open(f'parahome/s{subj_num}/object_transformations.pkl', 'rb')
            object_transformations = pickle.load(object_transformations)
            for i, static_object_name in enumerate(self.static_object_name):
                static_obj_pose.p.x = object_transformations[0][f'{static_object_name}_base'][0,3]
                static_obj_pose.p.y = object_transformations[0][f'{static_object_name}_base'][1,3]
                static_obj_pose.p.z = object_transformations[0][f'{static_object_name}_base'][2,3]
                quat = R.from_matrix([object_transformations[0][f'{static_object_name}_base'][:3, :3]]).as_quat()
                static_obj_pose.r = gymapi.Quat(quat[0,0], quat[0,1], quat[0,2], quat[0,3])
                static_target_handle = self.gym.create_actor(env_ptr, self._static_target_asset[i], static_obj_pose, static_object_name, col_group, col_filter, segmentation_id)
                self.static_target_handles.append(static_target_handle)
        return
