...
class InterMimic_GRAB(Humanoid_SMPLX):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self._state_init = InterMimic_GRAB.StateInit[state_init]
        ...
        self.object_name = [motion_example.split('_')[-3].split(',')[0] for motion_example in self.motion_file]
        self.spawn_objects_name = [motion_example.split('_')[-3].split(',') for motion_example in self.motion_file]
        self.all_spawn_objects_name = list(set(sum([spawn_objs for spawn_objs in self.spawn_objects_name], [])))
        object_name_set = list(set(self.object_name))
        ...
        self.dataset_index = to_torch([int(data_path.split('/')[-1].split('_')[0][1:]) for data_path in self.motion_file], dtype=torch.long).cuda()
    ...
    def _load_motion(self, motion_file, startk=0, topk=1, initk=0):
        ...
        for idx, data_path in enumerate(motion_file):
            ...
            loaded_dict['body_rot'] = loaded_dict['hoi_data'][:, 162+52*3:162+52*3+52*4].clone() # [frame, 208] # 318:526
            ...
            # ============ obj pos & rot ===============
            loaded_dict['obj_pos'] = loaded_dict['hoi_data'][:, 526:529].clone()
            ...
            loaded_dict['obj_rot'] = loaded_dict['hoi_data'][:, 529:533].clone()
            ...
            # ============ obj contact ===============
            loaded_dict['contact_obj'] = torch.round(loaded_dict['hoi_data'][:, 538:539].clone())
            loaded_dict['contact_human'] = torch.round(loaded_dict['hoi_data'][:, 539:539+52].clone())
    ...
    def _load_target_asset(self): # smplx
        ...
        asset_root = "isaacgym/src/intermimic/data/assets/grab/sim_object" # for InterMimic_GRAB
        self._target_asset = {}
        points_num = []
        self.object_points = []
        for i, obj_name in enumerate(self.all_spawn_objects_name):
            ...
            if obj_name in ['table', 'washingmachine', 'microwave', 'gasstove',
                            'sink', 'drawer', 'desk', 'diningtable', 'bookshelf']:
                asset_options.fix_base_link = True
            
            self._target_asset[obj_name] = self.gym.load_asset(self.sim, str(asset_root), asset_file, asset_options)
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        motion_file = self.motion_file[env_id % len(self.spawn_objects_name)]
        motion_data = torch.load(motion_file)['data']

        for obj_idx, object_name in enumerate(spawn_objects):

            default_pose = gymapi.Transform()
            if obj_idx>0:
                default_pose.p.x = motion_data[0, 526+65*obj_idx:529+65*obj_idx][0]
                default_pose.p.y = motion_data[0, 526+65*obj_idx:529+65*obj_idx][1]
                default_pose.p.z = motion_data[0, 526+65*obj_idx:529+65*obj_idx][2]
                quat = motion_data[0, 529+65*obj_idx:533+65*obj_idx]
                default_pose.r = gymapi.Quat(quat[0], quat[1], quat[2], quat[3])
    ...
    def _reset_actors(self, env_ids):
        if (self._state_init == InterMimic_GRAB.StateInit.Default):
            self._reset_default(env_ids)
        elif (self._state_init == InterMimic_GRAB.StateInit.Start
              or self._state_init == InterMimic_GRAB.StateInit.Random):
            self._reset_ref_state_init(env_ids)
        elif (self._state_init == InterMimic_GRAB.StateInit.Hybrid):
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        if (self._state_init == InterMimic_GRAB.StateInit.Random
            or self._state_init == InterMimic_GRAB.StateInit.Hybrid):
    ...
    def compute_humanoid_observations_max(self, body_pos, body_rot, body_vel, body_ang_vel, local_root_obs, root_height_obs, contact_forces, contact_body_ids, ref_obs, key_body_ids):
        ...
        # ----------- heading rot for body pos 52-----------
        heading_rot_expand = heading_rot_expand.repeat((1, body_pos.shape[1], 1))
        flat_heading_rot = heading_rot_expand.reshape(heading_rot_expand.shape[0] * heading_rot_expand.shape[1], 
                                                heading_rot_expand.shape[2])
        # ----------- heading rot for body pos - 2 hands -----------
        heading_rot_expand = heading_rot.unsqueeze(-2)
        ...
        # ----------- inv heading rot for body pos - 2 hands -----------
        heading_inv_rot_expand = heading_inv_rot.unsqueeze(-2)
        ...
        local_body_pos_obs = flat_local_body_pos.reshape(local_body_pos.shape[0], local_body_pos.shape[1] * local_body_pos.shape[2])
        local_body_pos_obs = local_body_pos_obs[..., 3:] # remove root pos
        ...
        # ============= contact (21 body + 10 finger tips) =================
        body_contact_buf = contact_forces[:, contact_body_ids, :].clone() #.view(contact_forces.shape[0],-1)
        contact = torch.any(torch.abs(body_contact_buf) > 0.1, dim=-1).float() # (num_envs, 31)
        ...
        obs = torch.cat((root_h_obs, local_body_pos_obs, local_body_rot_obs, local_body_vel, local_body_ang_vel, contact, diff_local_body_pos_flat, diff_local_body_rot_obs, diff_body_contact, local_ref_body_pos, local_ref_body_rot, diff_local_vel, diff_local_ang_vel), dim=-1)
        return obs
    ...
    def _compute_observations_iter(self, hoi_data, env_ids=None, delta_t=1):
        ...
        obs = self._compute_humanoid_obs(env_ids, ref_obs, next_ts) # [2048, 1359]
        task_obs = self._compute_task_obs(env_ids, ref_obs) # [2048, 21]
        obs = torch.cat([obs, task_obs], dim=-1)     # [2048, 1380]
