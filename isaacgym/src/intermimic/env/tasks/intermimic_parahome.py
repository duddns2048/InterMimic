...
class InterMimic_Parahome(Humanoid_SMPLX):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self._state_init = InterMimic_Parahome.StateInit[state_init]
        ...
        # self.object_name = [motion_example.split('_')[-2].split(',')[0] for motion_example in self.motion_file]
        self.object_name = [motion_example.split('_')[-2].split(',')[0] for motion_example in self.motion_file]
    ...
    def _load_motion(self, motion_file, startk=0, topk=1, initk=0):
        ...
        for idx, data_path in enumerate(motion_file):
            ...
            loaded_dict['body_pos'] = loaded_dict['hoi_data'][:, 162: 162+52*3].clone() # 162:318
        ...
        self.hoi_refs = torch.stack(self.hoi_refs, dim=0).unsqueeze(1).repeat(1, topk, 1, 1) # f, 332
    ...
    def _load_target_asset(self): # smplx
        asset_root = "isaacgym/src/intermimic/data/assets/parahome/sim_object" # for InterMimic_Parahome
        self._target_asset = {}
        self.object_points = []
        for i, obj_name in enumerate(self.all_spawn_objects_name):
            ...
            obj_file = f"{asset_root}/{obj_name}/base_collision.obj"
            import os
    ...
    def _reset_actors(self, env_ids):
        if (self._state_init == InterMimic_Parahome.StateInit.Default):
            self._reset_default(env_ids)
        elif (self._state_init == InterMimic_Parahome.StateInit.Start
              or self._state_init == InterMimic_Parahome.StateInit.Random):
            self._reset_ref_state_init(env_ids)
        elif (self._state_init == InterMimic_Parahome.StateInit.Hybrid):
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        if (self._state_init == InterMimic_Parahome.StateInit.Random
            or self._state_init == InterMimic_Parahome.StateInit.Hybrid):
